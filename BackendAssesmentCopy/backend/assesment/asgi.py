...
def jsonObjDecoder(obj):
    if '__type__' in obj and obj['__type__'] == 'TenantCreate':
        data=schemas.TenantCreate(obj['tenant_id'], obj['tenant_name'], obj['address'], obj['city'], obj['state'], obj['country'], obj['zip_code'], obj['phone'], obj['web_url'])	
    if '__type__' in obj and obj['__type__'] == 'UserCreate':
        data=schemas.UserCreate(obj['user_id'], obj['first_name'], obj['last_name'], obj['department'], obj['designation'], obj['tenant_id'], obj['city'], obj['country'], obj['bio'], obj['social_links'], obj['employee_id'])
    return data
...
def FuncTwo(db: Session = Depends(get_db)):
    ...
    print("Type",type(m))
    #if type(m) == 'Tenant':
        #classObj = crud.get_tenant_by_id(db, tenant_id=m.tenant_id)
        #if classObj:
             #raise HTTPException(status_code=400, detail="Record already exists")
        #try:
             #crud.create_tenant(db=db, tenant=m)
        #except Exception as error:
             #print(error, flush=True)
    #if className == 'UserCreate':
